...
class VIXPositionScale:
    ...
    def get_multiplier(self, vix: float) -> float:
        ...
        if vix is None or (isinstance(vix, float) and math.isnan(vix)) or vix < 0:
            logger.warning(
                f"VIX value invalid ({vix}): returning 0.0 (frozen/conservative)"
            )
            return 0.0
        for threshold, multiplier, regime in self.THRESHOLDS:
            if vix >= threshold:
                if multiplier == 0.0:
                    ...
                    logger.info(
                        f"VIX {vix:.1f}: {regime} regime, multiplier={multiplier}"
                    )
                return multiplier
        return 1.0
    ...
    def get_threshold_info(self) -> list:
        ...
        return [
            {"min_vix": t, "multiplier": m, "regime": r} for t, m, r in self.THRESHOLDS
        ]
