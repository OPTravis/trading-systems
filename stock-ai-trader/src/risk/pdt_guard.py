...
class PDTGuard:
    ...
    def record_day_trade(self, timestamp: Optional[datetime] = None):
        ...
        logger.info(
            f"Day trade recorded at {ts}. Rolling count: {self._count_day_trades()}"
        )
    ...
    def _count_day_trades(self) -> int:
        ...
        )  # safe upper bound covering weekends + holidays
        self.day_trades = [t for t in self.day_trades if t > cutoff]
        return len(self.day_trades)

    def get_remaining_day_trades(self, account_value: float) -> float:
