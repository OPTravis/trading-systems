...
# Phase 1: one-shot bootstrap (idempotent via marker). setup_crypto.sh
# carries git credentials and therefore intentionally lives OUTSIDE the
# repo — guard on its presence so a bare /root never breaks keepalive.
if [ ! -f /root/.crypto_bootstrapped ] && [ -f /root/setup_crypto.sh ]; then
  if bash /root/setup_crypto.sh >> "$REPO/logs/setup_crypto.log" 2>&1; then
