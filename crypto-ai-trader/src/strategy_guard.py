    ...
    def decorator(func: Callable[..., T]) -> Callable[..., T]:
        ...
        def wrapper(*args, **kwargs) -> T:
            ...
            if func_name not in _circuit_registry:
                _circuit_registry[func_name] = _CircuitBreaker(max_failures, cooldown_sec)
            ...
            try:
                ...
                if should_open:
                    logger.log(log_level, f"Circuit OPENED for {func_name} after {cb.failure_count} failures: {e}")
                else:
                    logger.log(log_level, f"Strategy error in {func_name} ({cb.failure_count}/{max_failures}): {e}")
        ...
        wrapper._circuit_breaker = lambda: _circuit_registry.get(func_name)
        wrapper._circuit_status = lambda: {
            "func": func_name,
