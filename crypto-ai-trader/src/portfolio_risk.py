...
class RiskMixin:
    ...
    _daily_start_date: Any
    _daily_start_value: Optional[float]
    ...
    def check_risk_limits(self) -> Dict:
        ...
        if self._daily_start_value and self._daily_start_value > 0:
            ...
            if daily_loss_pct < -self.config.get("max_daily_loss_pct", 100):
                warnings.append(
                    f"Daily loss {daily_loss_pct:.1f}% exceeds max {self.config['max_daily_loss_pct']}%"
                )
        ...
        for pos in self.positions.values():
            ...
            if pos_pct > self.config.get("max_position_pct", 100):
                warnings.append(
                    f"Position {pos['symbol']} at {pos_pct:.1f}% exceeds limit"
                )
                ...
                if created:
                    try:
                        ...
                        if hours_held > max_hold_hours:
                            warnings.append(
                                f"Max hold time exceeded for {pos['symbol']} ({hours_held:.1f}h > {max_hold_hours}h)"
                            )
                            positions_to_close.append(pos["symbol"])
                    except (ValueError, TypeError):
                        logger.debug(
                            "Could not parse created_at timestamp for %s: %s",
                            pos.get("symbol"),
                            created,
                            exc_info=True,
                        )
    ...
    def suggest_rebalance(self) -> Dict:
        ...
        return {
            ...
            "rebalance_needed": abs(target_exposure - current_exposure)
            > total_value * 0.05,
