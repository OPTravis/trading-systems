...
class PortfolioManager(PnlMixin, RiskMixin, StateMixin):
    ...
    def __init__(self, config_path: str = None, binance_client: 'ExchangeClient' = None):
        # Load risk config
    ...
    def _validate_config(self, config: Dict) -> Dict:
        ...
        for key in defaults:
            ...
            elif isinstance(defaults[key], dict) and not isinstance(config.get(key), dict):
                # Config value is wrong type (e.g. null), reset to default
                logger.warning(f"Config key '{key}' expected dict, got {type(config.get(key)).__name__} — using default")
                config[key] = defaults[key]
        return config
    ...
    def update_balance(self, balance: float):
        ...
        self._save_state(force=False)  # debounce — critical ops (add/close/sync) use force=True
    ...
    def add_position(
        ...
        if position_value < self.DUST_THRESHOLD_USD:
            logger.info(f"Dust position ignored: {symbol} {quantity} @ {entry_price} = ${position_value:.4f}")
            return
        ...
        if not _skip_validation:
            ...
            if max_positions is not None and symbol not in self.positions:
                non_dust = sum(
                    ...
                    if (p.get('current_price', 0) or p.get('entry_price', 0)) * p.get('quantity', 0) >= 5.0
                )
        ...
        if symbol in self.positions:
            ...
            self.positions[symbol] = {
                ...
                "strategy": strategy,
                "stop_loss": stop_loss if stop_loss is not None else old.get("stop_loss", new_entry * (1 - self.config["stop_loss"]["default_pct"] / 100)),
                "take_profit": take_profit if take_profit is not None else old.get("take_profit", new_entry * (1 + self.config.get("take_profit", {}).get("default_pct", 6.0) / 100)),
                "trailing_stop_pct": old.get("trailing_stop_pct", 1.5),
                "highest_price": max(old.get("highest_price", entry_price), entry_price),
                "created_at": old.get("created_at", now),
                "updated_at": now,
            }
            logger.info(f"Merged position: {symbol} -> {new_qty} @ {new_entry:.6f}")
        else:
            sl = stop_loss if stop_loss is not None else entry_price * (1 - self.config["stop_loss"]["default_pct"] / 100)
            tp = take_profit if take_profit is not None else entry_price * (1 + self.config.get("take_profit", {}).get("default_pct", 6.0) / 100)
            self.positions[symbol] = {
                "symbol": symbol,
                "quantity": quantity,
        ...
        if self._db is not None and not _from_sync:
            try:
                ...
                logger.info(f"Recorded trade: {symbol} BUY qty={quantity} price={entry_price}")
            except Exception as e:
    ...
    def close_position(self, symbol: str, close_price: float = None) -> Dict:
        ...
        if symbol in self.positions:
            ...
            price = close_price if close_price is not None else pos.get("current_price", pos["entry_price"])
            ...
            if self._db is not None:
                try:
                    ...
                    logger.info(f"Recorded trade: {symbol} SELL qty={pos['quantity']} price={price:.6f} PnL={pnl:.2f}")
                except Exception as e:
                    logger.error(f"Failed to record trade to StateDB: {e}")
            ...
            try:
                ...
                ctx = {"hmm_regime": "sideways", "fear_greed": 50, "btc_trend": "NEUTRAL", "portfolio_heat": "warm"}
    ...
    def get_all_positions(self) -> List[Dict]:
        ...
        for pos in self.positions.values():
            ...
            if "take_profit" not in enriched or enriched["take_profit"] is None:
                tp_default = pos["entry_price"] * (1 + self.config.get("take_profit", {}).get("default_pct", 6.0) / 100)
            ...
            enriched["entry_price"] = pos.get("entry_price", enriched.get("entry_price", current_price))
