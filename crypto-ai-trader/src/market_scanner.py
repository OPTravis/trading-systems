...
class MarketScanner:
    ...
    def scan_all(self) -> List[Dict]:
        ...
        try:
            ...
            _real_positions = [
                {"symbol": sym, "quantity": data["quantity"], "entry_price": data["entry_price"]}
                for sym, data in db_positions.items()
            ]
        except Exception:
            logger.error("Failed to load real positions from DB for pool filtering", exc_info=True)
            _real_positions = []
    ...
    def get_top_movers(self, limit: int = 10) -> List[Dict]:
        ...
        losers = sorted(
            usdt_pairs, key=lambda x: float(x.get("price_change_pct", 0))
        )[:limit]
    ...
    def _analyze_coin(self, coin_data: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            if len(klines_1h) >= 21:
                ...
                elapsed_min = ((_time_mod.time() * 1000) - candle_open_time) / 60000 if candle_open_time else 30
            ...
            try:
                ...
                logger.error("Consolidation breakout analysis failed for %s", symbol, exc_info=True)
        except Exception:
        ...
        try:
            ...
            logger.error("Fear & Greed Index fetch failed, defaulting to 50", exc_info=True)
            fng_value = 50
        ...
        try:
            ...
            logger.error("On-chain score fetch failed, defaulting to 50.0", exc_info=True)
            onchain_score = 50.0
        ...
        try:
            ...
            tech_result = TechnicalAgent().analyze(
                tf_1h=tf_1h_raw,
                bb_squeeze_data=new_signals_data.get("bb_squeeze") if new_signals_data else None,
                rsi_div_data=new_signals_data.get("rsi_div") if new_signals_data else None,
            )
        ...
        try:
            vol_result = VolumeAgent().analyze(coin_data=coin_data, volume_surge=volume_surge)
        ...
        market_sentiment_score = fng_result.score if 'market_sentiment' in agent_scores else 50.0
        ...
        if 'technical' in agent_scores and hasattr(agent_scores['technical'], 'data'):
            technical_score = agent_scores['technical'].data.get('f_technical', self._factor_technical(tf_1h))
        else:
        ...
        return {
            ...
            "trend_strength": mtf_result.get("trend_score", 0),  # alias for backward compat
            "atr_15m": mtf_result.get("atr_15m", 0),
            "technical_score": round(technical_score, 1),
            "sentiment_score": sentiment_data.get("sentiment_score") if sentiment_data else None,
            "funding_rate": sentiment_data.get("funding_rate") if sentiment_data else None,
            "oi_change_pct": sentiment_data.get("oi_change_pct") if sentiment_data else None,
            "market_sentiment_score": round(market_sentiment_score, 1),
            "onchain_score": round(onchain_score, 1),
            "predicted_direction": predicted_direction,
            "prediction_confidence": round(prediction_confidence, 3) if prediction_confidence else None,
            "analysis": {
                "1h": tf_1h,
                "4h": tf_4h,
                "15m": tf_15m,
    ...
    def _calculate_weighted_score(
        ...
        f7 = pp_data.get('f_obv_divergence', self._factor_obv_divergence(
            (new_signals_data or {}).get("obv_div")))

        # Factor 7: Consolidation Breakout (8%) — from PrePumpAgent
        f8 = pp_data.get('f_consolidation', self._factor_consolidation(
            (new_signals_data or {}).get("consolidation")))

        # Factor 8: BB Squeeze (4%) — from TechnicalAgent
        f9 = tech_data.get('f_bb_squeeze', self._factor_bb_squeeze(
            (new_signals_data or {}).get("bb_squeeze")))

        # Factor 9: RSI Divergence (4%) — from TechnicalAgent
        f10 = tech_data.get('f_rsi_divergence', self._factor_rsi_divergence(
            (new_signals_data or {}).get("rsi_div")))
        ...
        if self._learner_weights is not None and (_now - self._learner_ts) < 300:
            ...
            try:
                ...
                logger.error("Failed to load learned weights from OnlineLearner, falling back to defaults", exc_info=True)
                _w = {
                    "technical": 16.0, "trend": 16.0, "volume": 11.0,
                    "sentiment": 9.0, "price_action": 8.0, "obv_divergence": 8.0,
                    "consolidation": 8.0, "bb_squeeze": 4.0, "rsi_divergence": 4.0,
                    "onchain": 8.0, "market_sentiment": 5.0, "orderbook": 3.0,
                }
                self._learner_weights = _w
                self._learner_ts = _now
    ...
    def _generate_signals(
        ...
        if new_signals_data:
            ...
            if obv_div and obv_div.get("detected"):
                signals.append(f"🐋 OBV Bullish Divergence (strength: {obv_div['strength']:.0f})")
            ...
            if consol and consol.get("breaking_out"):
                ...
                signals.append(f"🚀 Consolidation Breakout ({consol['days_in_range']}d range{vol_tag})")
            elif consol and consol.get("in_consolidation"):
                signals.append(f"📦 In Consolidation ({consol['days_in_range']}d, {consol['range_pct']:.1f}% range)")

            # BB Squeeze
            squeeze = new_signals_data.get("bb_squeeze")
            if squeeze and squeeze.get("squeezing"):
                signals.append(f"⚡ BB Squeeze (percentile: {squeeze['percentile']:.0f}%)")

            # RSI Divergence
            rsi_div = new_signals_data.get("rsi_div")
            if rsi_div and rsi_div.get("detected"):
                signals.append(f"💎 RSI Bullish Divergence (strength: {rsi_div['strength']:.0f})")
