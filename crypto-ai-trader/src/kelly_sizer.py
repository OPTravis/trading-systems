...
class KellyPositionSizer:
    ...
    def _exploration_entries_last_30d(self) -> int:
        """Count regime-warming exploration entries in the last 30 days.

        Slow-bleed guard: if F&G hovers 40-50 while the market drifts
        down, every closed position would re-trigger another exploratory
        entry — cumulative drag plus statistical noise. The confidence
        string is persisted in trade_outcomes.context_json, so one query
        traces which entries came from this mechanism.
        """
        if not self.db:
            return 0
        try:
