...
class StrategyAdaptor:
    ...
    def _build_strategy_configs(
        ...
        for name, config in self.STRATEGIES.items():
            ...
            if regime in ("EXTREME_FEAR", "FEAR"):
                if name == "grid":
                    # Re-enable grid in FEAR when market is range-bound (low ADX + low vol)
                    # Grid is optimal for sideways markets — ADX<25 confirms no trend
                    _adx = btc_adx if btc_adx is not None else 50
                    if _adx < 25 and vol_regime in ("LOW", "NORMAL"):
                        enabled = True
                        size_mult = 0.7  # Conservative sizing in fear
                        reason = "enabled — fear + range-bound (ADX<{:.0f}, vol={})".format(_adx, vol_regime)
                        changes.append(f"{name}: enabled ×0.7 — fear + range-bound (ADX {_adx:.0f}<25)")
                    else:
                        enabled = False
                        reason = "disabled in fear (ADX={:.0f}, vol={})".format(_adx, vol_regime)
                        changes.append(f"{name}: disabled — fear regime (ADX {_adx:.0f})")
                elif name == "trend":
                    enabled = False
