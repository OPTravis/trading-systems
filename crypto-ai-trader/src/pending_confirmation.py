...
def load_pending() -> Optional[Dict]:
    ...
    try:
        ...
        if age_hours > ttl:
            ...
            logger.info(f"Pending confirmation expired (age: {age_hours:.1f}h > {ttl}h)")
            return None
...
def clear_pending(symbol: str = None) -> bool:
    ...
    try:
        ...
        if data.get("symbol") == symbol:
            ...
            logger.info(f"Pending is for {data.get('symbol')}, not {symbol} — not clearing")
            return False
    except Exception as e:
...
if __name__ == "__main__":
    ...
    test_data = {
        ...
        "stop_loss_pct": 2.0,
        "tp_levels": [{"pct": 2.0, "size_pct": 33}, {"pct": 3.0, "size_pct": 33}, {"pct": 5.0, "size_pct": 34}],
        "stop_price": 0.009163,
        "max_hold_hours": 24
    }
