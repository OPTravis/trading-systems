...
class SmartOrder:
    ...
    def place_buy_with_sl_tp(
        ...
        if quantity <= 0:
            return {
                "success": False,
                "reason": "Position size too small or limits reached",
            }
        ...
        if not filled_symbol_filters:
            logger.warning(
                f"No filters for {full_symbol}, using defaults for order sizing"
            )
            filled_symbol_filters = {
                "stepSize": 0.001,
                "minQty": 0.0,
                "maxQty": 999999999.0,
                "qty_decimals": 4,
        ...
        if sl_qty <= 0 and filled_qty > tp1_qty + tp2_qty:
            ...
            if remainder > 0:
                ...
                if tp2_qty > 0:
                    tp2_qty = self.apply_qty_precision(
                        tp2_qty + remainder, filled_symbol_filters
                    )
                elif tp1_qty > 0:
                    tp1_qty = self.apply_qty_precision(
                        tp1_qty + remainder, filled_symbol_filters
                    )
                else:
        ...
        if tp1_qty > 0 and tp1_qty * sl_tp["tp1_price"] < min_notional:
            logger.warning(
                f"TP1 merged qty still below minNotional, zeroing for {symbol}"
            )
            tp1_qty = 0.0
        if tp2_qty > 0 and tp2_qty * sl_tp["tp2_price"] < min_notional:
            logger.warning(
                f"TP2 merged qty still below minNotional, zeroing for {symbol}"
            )
            tp2_qty = 0.0
        ...
        tp1_result = None
        if tp1_qty > 0:
            tp1_result = self.client.place_limit_sell(
                full_symbol, tp1_qty, sl_tp["tp1_price"]
            )

        tp2_result = None
        if tp2_qty > 0:
            tp2_result = self.client.place_limit_sell(
                full_symbol, tp2_qty, sl_tp["tp2_price"]
            )
        ...
        if tp3_qty > 0:
            ...
            if tp3_notional >= min_notional:
                tp3_result = self.client.place_limit_sell(
                    full_symbol, tp3_qty, sl_tp["tp3_price"]
                )
                if tp3_result:
                    ...
                    logger.info(
                        "TP3 placed for %s: qty=%s @ $%s (6x ATR runner)",
                        symbol,
                        tp3_qty,
                        round(sl_tp["tp3_price"], 6),
                    )
                else:
                    logger.warning(f"TP3 order failed for {symbol}")
            else:
                logger.info(
                    "TP3 skipped for %s: notional $%s < minNotional $%s",
                    symbol,
                    round(tp3_notional, 2),
                    min_notional,
                )
        ...
        if not sl_result:
            logger.error(
                f"CRITICAL: SL order failed for {symbol} — position is unprotected!"
            )
        if not tp1_result:
        ...
        return {
            ...
            "risk_reward": (
                f"1:{round(sl_tp['tp1_pct'] / sl_tp['sl_pct'], 1)}"
                if sl_tp["sl_pct"] > 0
                else "N/A"
            ),
            "sl_order": sl_result,
