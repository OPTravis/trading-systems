...
class DCAStrategy(BaseStrategy):
    ...
    def __init__(self, params: Dict):
        ...
        if self.interval_hours <= 0:
            logger.warning(
                f"DCA interval_hours must be positive, got {self.interval_hours}, using default 24"
            )
            self.interval_hours = 24
        if self.max_dca_rounds < 1:
            logger.warning(
                f"DCA max_dca_rounds must be >= 1, got {self.max_dca_rounds}, using default 5"
            )
        ...
        if current_qty == 0:
            ...
            if deviation_pct <= self.dip_threshold_pct:
                ...
                return StrategySignal(
                    signal=SignalType.BUY,
                    confidence=60,
                    reason="Downtrend detected - start DCA",
                    metadata={
            ...
            if (
                ...
                return StrategySignal(
                    ...
                    confidence=60,
                    reason="Profitable DCA - take partial profits",
                    metadata={"sell_pct": 25, "deviation_pct": deviation_pct},
                )
            else:
                return StrategySignal(
