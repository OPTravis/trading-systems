...
class BollingerStrategy(BaseStrategy):
    ...
    def __init__(self, params: Dict):
        ...
        if self.period < 2:
            logger.warning(
                f"Bollinger period must be >= 2, got {self.period}, using default 20"
            )
            self.period = 20
        if self.std_dev <= 0:
            logger.warning(
                f"Bollinger std_dev must be > 0, got {self.std_dev}, using default 2.0"
            )
            self.std_dev = 2.0
    ...
    def analyze(
        ...
        band_position = (
            (current_price - bb["lower"]) / bandwidth if bandwidth > 0 else 0.5
        )
        ...
        if current_qty == 0:
            ...
            if current_price > bb["upper"] and volume_ratio >= self.volume_threshold:
                return StrategySignal(
                    ...
                    confidence=75,
                    reason="Breakout above upper band with volume",
                    metadata={
                        "bb_upper": bb["upper"],
                        "bb_middle": bb["middle"],
                        "bb_lower": bb["lower"],
                ...
                return StrategySignal(
                    ...
                    metadata={"bb_lower": bb["lower"], "band_position": band_position},
                )
            else:
                return StrategySignal(
                    signal=SignalType.WAIT,
                    ...
                    metadata={"band_position": band_position},
                )
        else:
            # Have position
            ...
            if band_position < 0.35 and pnl_pct > 0:
                ...
                return StrategySignal(
                    ...
                    metadata={"pnl_pct": pnl_pct, "band_position": band_position},
                )
