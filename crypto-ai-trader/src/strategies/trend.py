...
class TrendStrategy(BaseStrategy):
    ...
    def __init__(self, params: Dict):
        ...
        if self.fast_ma < 2 or self.slow_ma < 2:
            logger.warning(
                f"Trend MA periods must be >= 2, got fast={self.fast_ma} slow={self.slow_ma}, using defaults 9/21"
            )
            self.fast_ma, self.slow_ma = 9, 21
        if self.fast_ma >= self.slow_ma:
            logger.warning(
                f"Trend fast_ma ({self.fast_ma}) must be < slow_ma ({self.slow_ma}), using defaults 9/21"
            )
            self.fast_ma, self.slow_ma = 9, 21
        ...
        if current_qty == 0:
            ...
            if golden_cross and volume_ratio >= self.volume_threshold:
                ...
                return StrategySignal(
                    ...
                    metadata={"fast_ma": fast_ma_val, "slow_ma": slow_ma_val},
                )
        else:
            ...
            if death_cross:
                ...
                return StrategySignal(
                    ...
                    metadata={"pnl_pct": pnl_pct, "sell_pct": 50},  # Partial sell
                )
