...
class TestTypeErrors:
    ...
    def test_signal_type_comparison_in_run_all(self):
        ...
        # 不應有 string 比較
        assert (
            'signal.signal in ("BUY"' not in source
        ), "run_all 仍然用 string 比較信號！應使用 SignalType.BUY"
        assert (
            "SignalType.BUY" in source or "SignalType.SELL" in source
...
class TestEmptyData:
    ...
    def test_score_range_0_to_100(self):
        ...
        ta = TechnicalAgent()
        result = ta.analyze([])
        assert 0 <= result.score <= 100, f"TechnicalAgent score={result.score} 超出範圍"
...
class TestDataSource:
    ...
    def test_kelly_reads_trade_outcomes(self):
        ...
        assert "trade_outcomes" in source, "_get_trade_history 必須讀 trade_outcomes 表"
        assert "FROM trades" not in source, "_get_trade_history 不應讀 trades 表"
            ...
            if isinstance(node, ast.FunctionDef):
                ...
                if func_source and "db." in func_source:
                    ...
                    has_db_assign = any(
                        ...
                        and any(
                            isinstance(t, ast.Name) and t.id == "db" for t in n.targets
                        )
...
class TestIntegration:
    ...
    def test_ensure_tp_sl_runs(self):
        ...
        assert result.returncode == 0, f"ensure_tp_sl.py 失敗:\n{result.stderr[:500]}"
    ...
    def test_auto_heal_runs(self):
        ...
        assert result.returncode >= 0, f"auto_heal.py 被殺死:\n{result.stderr[:500]}"
