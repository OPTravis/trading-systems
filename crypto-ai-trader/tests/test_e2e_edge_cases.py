...
def _make_bc_for_positions(balances, price_map=None):
    ...
    default_prices = {"BTCUSDT": "50000", "ETHUSDT": "3000", "SOLUSDT": "100", "DOGEUSDT": "0.1"}
    if price_map:
...
class TestExecuteAutoTradeEdgeCases:
    ...
    def _run(self, score=75, usdt_bal=1000, active_positions=0, tps=None, filters=None):
        ...
        bc.place_market_buy.return_value = {
            "symbol": "SOLUSDT", "orderId": 999, "status": "FILLED",
            "fills": [{"price": "100.00", "qty": "10", "commission": "0.01"}],
        }
        bc.place_order.return_value = {"symbol": "SOLUSDT", "orderId": 1000, "status": "NEW"}
        notifier = MagicMock()
        notifier.get_strategy_config.return_value = {
            "stop_loss_pct": 2.0, "max_hold_hours": 24,
            "take_profit_levels": [{"pct": 2.0, "size_pct": 50}, {"pct": 5.0, "size_pct": 50}],
        }
        so = MagicMock()
        so.get_symbol_filters.return_value = filters or {"stepSize": 1.0, "qty_decimals": 0, "minQty": 1.0, "minNotional": 5.0}
        _tps = tps or [{"pct": 2.0, "size_pct": 33}, {"pct": 3.0, "size_pct": 33}, {"pct": 5.0, "size_pct": 34}]
        with patch("src.trade_executor.get_trading_client", return_value=bc), \
            ...
            result = execute_auto_trade(
                "SOLUSDT", 100.0, "trend", 2.0, _tps,
                98.0, 24, ["RSI"], "RSI", score=score,
            )
        return result, bc

    def test_s12_tp_total_over_70_scaled(self):
        tps = [{"pct": 2.0, "size_pct": 40}, {"pct": 5.0, "size_pct": 40}, {"pct": 8.0, "size_pct": 30}]
    ...
    def test_s14_buy_no_fills(self):
        ...
        so = MagicMock()
        so.get_symbol_filters.return_value = {"stepSize": 1.0, "qty_decimals": 0, "minQty": 1.0, "minNotional": 5.0}
        with patch("src.trade_executor.get_trading_client", return_value=bc), \
             patch("src.trade_executor.FeishuNotifier", return_value=notifier), \
            ...
            result = execute_auto_trade(
                ...
                [{"pct": 2.0, "size_pct": 50}], 98.0, 24, ["RSI"], "RSI", score=75,
            )
        assert result["success"] is True
        # When no fills, executed_qty = qty from args

    def test_s15_stepsize_rounding(self):
        result, bc = self._run(filters={"step_size": 0.01, "qty_decimals": 2, "min_qty": 0.01})
        assert result["success"] is True

    def test_s16_qty_below_minqty_error(self):
        result, _ = self._run(usdt_bal=50, filters={"min_qty": 100.0, "step_size": 1.0, "qty_decimals": 0})
    ...
    def test_s17_scale_monotonically_decreasing(self):
        ...
        for n in range(5):
            if n == 0:
                scales.append(1.0)
            elif n == 1: scales.append(0.75)
            elif n == 2: scales.append(0.5)
            elif n == 3: scales.append(0.3)
            elif n == 4: scales.append(0.2)
        # Verify monotonically decreasing
        for i in range(len(scales) - 1):
            assert scales[i] > scales[i + 1], f"Scale {i} ({scales[i]}) not > scale {i+1} ({scales[i+1]})"
...
class TestSLTPOrderInteraction:
    ...
    def _run_with_tracking(self, usdt_bal=1000, min_notional=5.0):
        ...
        def mock_place(*a, **kw):
            ...
            return {"symbol": "SOLUSDT", "orderId": len(order_log) + 1000, "status": "NEW"}
        bc.place_order.side_effect = mock_place
        notifier = MagicMock()
        notifier.get_strategy_config.return_value = {
            "stop_loss_pct": 2.0, "max_hold_hours": 24,
            "take_profit_levels": [{"pct": 2.0, "size_pct": 33}, {"pct": 3.0, "size_pct": 33}, {"pct": 5.0, "size_pct": 34}],
        }
        so = MagicMock()
        so.get_symbol_filters.return_value = {"stepSize": 1.0, "qty_decimals": 0, "minQty": 1.0, "minNotional": min_notional}
        with patch("src.trade_executor.get_trading_client", return_value=bc), \
             patch("src.trade_executor.FeishuNotifier", return_value=notifier), \
             patch("src.trade_executor.count_active_positions", return_value=0), \
             patch("src.smart_order.SmartOrder", return_value=so), \
    ...
    def test_s19_sl_before_tp_call_order(self):
        ...
        assert result["success"] is True
        sell_orders = [o for o in order_log if len(o["args"]) > 1 and o["args"][1] == "SELL"]
        if len(sell_orders) >= 2:
            first_type = sell_orders[0]["args"][2] if len(sell_orders[0]["args"]) > 2 else sell_orders[0]["kwargs"].get("type", "")
            assert "STOP" in str(first_type), f"First sell order should be SL (STOP), got {first_type}"

    def test_s20_sl_covers_30pct_minimum(self):
        result, order_log = self._run_with_tracking()
        assert result["success"] is True
        executed_qty = 10.0
        sl_orders = [o for o in order_log if len(o["args"]) > 2 and "STOP" in str(o["args"][2])]
        if sl_orders:
            total_sl_qty = sum(o["args"][3] for o in sl_orders if len(o["args"]) > 3)
            assert total_sl_qty >= executed_qty * 0.3, f"SL qty {total_sl_qty} < 30% of {executed_qty}"
...
class TestCronScanIntegration:
    ...
    def _run_scan(self, bc, opportunities, sa_fng=50, rm_allowed=True, rm_reasons=None):
        ...
        sa = MagicMock()
        sa.get_market_sentiment.return_value = {"fear_greed": sa_fng, "fng_classification": "Neutral"}
        rm = MagicMock()
        rm.pre_trade_check.return_value = {"allowed": rm_allowed, "reasons": rm_reasons or [], "adjustments": {"size_multiplier": 1.0}}
        rm.trend_filter.check_trend.return_value = {
            "trend": "NEUTRAL", "score": 50, "adx": 25,
            "allow_long": True, "size_multiplier": 1.0, "factors": {},
        }
        notifier = MagicMock()
        notifier.get_strategy_config.return_value = {"stop_loss_pct": 2.0, "take_profit_levels": [{"pct": 2.0, "size_pct": 50}], "max_hold_hours": 24}
        with patch("src.scan_orchestrator.BinanceClient", return_value=bc), \
             ...
             patch.dict(os.environ, {"AUTO_EXECUTE": "true"}):
    ...
    def test_s23_scan_to_execute_flow(self):
        bc = MagicMock()
        bc.client.account.return_value = {"balances": [{"asset": "USDT", "free": "1000", "locked": "0"}]}
        ...
        assert call_kwargs[1].get("symbol") == "SOLUSDT" or call_kwargs[0][0] == "SOLUSDT"
...
class TestRiskManagerCrossModule:
    ...
    def test_s27_double_block_two_reasons(self, tmp_path):
        ...
        try:
            ...
            klines = [{"open": c, "high": c + 10, "low": c - 10, "close": c, "volume": 1000} for c in closes]
...
class TestDataIntegrity:
    ...
    def test_s31_trailing_state_persists(self, tmp_path):
        ...
        try:
            ...
            ts1.update("BTC", current_price=51500.0, atr=1000.0, entry_price=50000.0)  # activate
