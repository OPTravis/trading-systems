...
def test_llm_client_import():
    ...
    try:
        from src.llm_client import get_llm_client, get_second_opinion_client
...
def test_second_opinion_client():
    ...
    try:
        ...
        print(f"   ✅ Second opinion client created: {client._primary_cfg.get('provider')}")
        return True
    except Exception as e:
...
def test_structured_scoring():
    ...
    try:
        ...
        assert 1 <= result["score"] <= 10, f"Score should be 1-10, got {result['score']}"
        assert 0.0 <= result["confidence"] <= 1.0, f"Confidence should be 0.0-1.0, got {result['confidence']}"
        assert -1.0 <= result["sentiment"] <= 1.0, f"Sentiment should be -1.0 to 1.0, got {result['sentiment']}"
        
        print(f"   ✅ Structured scoring works: score={result['score']}, confidence={result['confidence']}")
        return True
    except Exception as e:
...
def test_cross_verification():
    ...
    try:
        ...
        assert result[0]["confidence"] == 0.9, f"Should be HIGH confidence (0.9), got {result[0]['confidence']}"
    ...
    try:
        ...
        for i, s in enumerate(old_format):
            if isinstance(s, (int, float)):
                ...
                sentiment = round(s, 2)
                assert -1.0 <= sentiment <= 1.0, f"Sentiment should be -1.0 to 1.0, got {sentiment}"
