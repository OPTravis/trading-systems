...
def _make_bc(usdt_free=1000, usdt_locked=0, extra_balances=None, scan_symbol="SOLUSDT"):
    ...
    bc.place_market_buy.return_value = {
        "symbol": scan_symbol, "orderId": 999, "status": "FILLED",
        "fills": [{"price": "100.00", "qty": "10", "commission": "0.01"}],
    }
    bc.place_order.return_value = {"symbol": scan_symbol, "orderId": 1000, "status": "NEW"}
    return bc
...
def _make_sentiment(fng=50, label="Neutral"):
    sa = MagicMock()
    sa.get_market_sentiment.return_value = {"fear_greed": fng, "fng_classification": label}
    return sa
...
def _make_rm(allowed=True, reasons=None, size_multiplier=1.0):
    ...
    rm.pre_trade_check.return_value = {
        "allowed": allowed, "reasons": reasons or [],
    ...
    rm.trend_filter.check_trend.return_value = {
        "trend": "NEUTRAL", "score": 50, "adx": 25,
        "allow_long": True, "size_multiplier": 1.0, "factors": {},
    }
    return rm
...
def _so_filters(step_size=1.0, qty_decimals=0, min_qty=1.0, min_notional=5.0):
    ...
    so.get_symbol_filters.return_value = {
        "stepSize": step_size, "qty_decimals": qty_decimals,
...
class TestCronScan:
    ...
    def _run_scan(self, bc, ms, sa, rm, notifier, auto_execute="true"):
        with patch("src.scan_orchestrator.BinanceClient", return_value=bc), \
             patch("src.scan_orchestrator.MarketScanner", return_value=ms), \
             ...
             patch("src.strategy_adaptor.StrategyAdaptor") as mock_sa, \
             ...
             patch.dict(os.environ, {"AUTO_EXECUTE": auto_execute}):
            mock_mr.return_value.research.return_value = {
                "score_adjustment": 0.0, "confidence": 0.5,
                "sentiment_summary": "mock", "news": [],
                "catalysts": [], "onchain": {},
            }
            mock_sa.return_value.adapt.return_value = {
                "regime": "NEUTRAL",
                "global": {"score_threshold": 60, "funding_signal": "N/A",
                           "cash_reserve_pct": 30, "max_position_pct": 15,
                           "max_total_exposure_pct": 70},
                "strategies": {},
            }
            mock_opt.return_value.analyze_and_switch.return_value = []
            mock_exec.return_value = {
                "success": True, "qty": 10.0, "tier": "MEDIUM-HIGH",
                "invest_pct": 29.7, "error": None,
            }
    ...
    def test_s1_happy_path_auto_execute(self):
        bc = _make_bc()
        opp = {"symbol": "SOLUSDT", "price": 100.0, "score": 75, "signals": ["RSI Oversold"], "atr": 5.0}
        mock_exec, _, _ = self._run_scan(bc, _make_scanner([opp]), _make_sentiment(50), _make_rm(True), _make_notifier(), "true")
        mock_exec.assert_called_once()

    def test_s2_extreme_fear_threshold_60(self):
        bc = _make_bc()
        opp = {"symbol": "DOGEUSDT", "price": 0.1, "score": 63, "signals": ["MACD"], "atr": 0.005}
        mock_exec, _, _ = self._run_scan(bc, _make_scanner([opp]), _make_sentiment(15, "Extreme Fear"), _make_rm(True), _make_notifier(), "true")
        mock_exec.assert_called_once()

    def test_s3_extreme_greed_threshold_85(self):
        bc = _make_bc()
        opp = {"symbol": "BTCUSDT", "price": 50000, "score": 80, "signals": ["Trend"], "atr": 500}
        mock_exec, _, mock_clear = self._run_scan(bc, _make_scanner([opp]), _make_sentiment(90, "Extreme Greed"), _make_rm(True), _make_notifier(), "true")
        # Extreme greed no longer blocks trades — regime adapts dynamically
        mock_exec.assert_called_once()

    def test_s4_fng_api_failure_fallback(self):
        bc = _make_bc()
        opp = {"symbol": "SOLUSDT", "price": 100, "score": 72, "signals": ["VWAP"], "atr": 5}
        sa = MagicMock()
        sa.get_market_sentiment.side_effect = Exception("API down")
        mock_exec, _, _ = self._run_scan(bc, _make_scanner([opp]), sa, _make_rm(True), _make_notifier(), "true")
        mock_exec.assert_called_once()

    def test_s5_trendfilter_bearish_blocked(self, capsys):
        bc = _make_bc()
        opp = {"symbol": "SOLUSDT", "price": 100, "score": 75, "signals": ["RSI"], "atr": 5}
        mock_exec, _, _ = self._run_scan(bc, _make_scanner([opp]), _make_sentiment(50), _make_rm(False, ["BTC trend is BEARISH, longs not allowed"]), _make_notifier(), "true")
    ...
    def test_s6_loss_guard_paused(self, capsys):
        bc = _make_bc()
        opp = {"symbol": "SOLUSDT", "price": 100, "score": 75, "signals": ["RSI"], "atr": 5}
        mock_exec, _, _ = self._run_scan(bc, _make_scanner([opp]), _make_sentiment(50), _make_rm(False, ["Consecutive loss guard active"]), _make_notifier(), "true")
        mock_exec.assert_not_called()
        assert "RISK_BLOCKED" in capsys.readouterr().out

    def test_s7_sector_blocked(self, capsys):
        bc = _make_bc()
        opp = {"symbol": "FETUSDT", "price": 5, "score": 80, "signals": ["MACD"], "atr": 0.2}
        mock_exec, _, _ = self._run_scan(bc, _make_scanner([opp]), _make_sentiment(50), _make_rm(False, ["Sector AI at 40%"]), _make_notifier(), "true")
        mock_exec.assert_not_called()
        assert "RISK_BLOCKED" in capsys.readouterr().out

    def test_s8_no_opportunities(self, capsys):
        bc = _make_bc()
        _, _, mock_clear = self._run_scan(bc, _make_scanner([]), _make_sentiment(50), _make_rm(), _make_notifier(), "true")
    ...
    def test_s9_already_holding_filtered(self, capsys):
        ...
        opp = {"symbol": "SOLUSDT", "price": 100, "score": 80, "signals": ["RSI"], "atr": 5}
        mock_exec, _, _ = self._run_scan(bc, _make_scanner([opp]), _make_sentiment(50), _make_rm(), _make_notifier(), "true")
    ...
    def test_s10_size_multiplier_stored(self):
        bc = _make_bc()
        opp = {"symbol": "SOLUSDT", "price": 100, "score": 75, "signals": ["RSI"], "atr": 5}
        mock_exec, _, _ = self._run_scan(bc, _make_scanner([opp]), _make_sentiment(50), _make_rm(True, size_multiplier=0.5), _make_notifier(), "true")
        mock_exec.assert_called_once()

    def test_s11_auto_execute_false_save_pending(self, capsys):
        bc = _make_bc()
        opp = {"symbol": "SOLUSDT", "price": 100, "score": 75, "signals": ["RSI"], "atr": 5}
        mock_exec, mock_save, _ = self._run_scan(bc, _make_scanner([opp]), _make_sentiment(50), _make_rm(True), _make_notifier(), "false")
        mock_exec.assert_not_called()
...
class TestExecuteAutoTrade:
    ...
    def _run(self, symbol="SOLUSDT", price=100.0, score=75, usdt_bal=1000,
             active_positions=0, tp_levels=None, filters=None, buy_result=None):
        ...
        with patch("src.trade_executor.get_trading_client", return_value=bc), \
             patch("src.trade_executor.FeishuNotifier", return_value=notifier), \
             patch("src.trade_executor.count_active_positions", return_value=active_positions), \
             patch("src.smart_order.SmartOrder", return_value=so), \
             patch("src.trade_executor.PortfolioManager"), \
             patch("src.circuit_breaker.CircuitBreaker") as mock_cb, \
             patch("src.daily_loss_breaker.get_daily_loss_breaker") as mock_dlb, \
             patch("src.drawdown_breaker.DrawdownBreaker") as mock_ddb, \
             patch("src.kelly_sizer.KellyPositionSizer") as mock_kelly, \
             patch("src.fee_optimizer.FeeOptimizer") as mock_fee, \
            ...
            mock_kelly.return_value.get_position_size.return_value = {
                "position_pct": 0, "confidence": "estimated",
            }
            from main import execute_auto_trade
            result = execute_auto_trade(
                symbol, price, "trend", 2.0, tps,
                price * 0.98, 24, ["RSI Oversold"], "RSI Oversold", score=score,
            )
        return result, bc
    ...
    @pytest.mark.parametrize("active,expected_scale", [
        (1, 0.80), (2, 0.65), (3, 0.50), (4, 0.35),
    ])
    ...
    def test_s17_market_buy_fails(self):
        ...
        with patch("src.trade_executor.get_trading_client", return_value=bc), \
             ...
             patch("src.trade_executor.PortfolioManager"):
            from main import execute_auto_trade
            result = execute_auto_trade("SOLUSDT", 100.0, "trend", 2.0, [{"pct": 2.0, "size_pct": 33}, {"pct": 3.0, "size_pct": 33}, {"pct": 5.0, "size_pct": 34}], 98.0, 24, ["RSI"], "RSI", score=75)
        assert result["success"] is False
    ...
    def test_s18_sl_placed_tp_partial_fail(self):
        ...
        def mock_place(*a, **kw):
            call_n[0] += 1
            return {"symbol": "SOLUSDT", "orderId": call_n[0] + 1000, "status": "NEW"} if call_n[0] <= 2 else None
        bc.place_order.side_effect = mock_place
        tps = [{"pct": 2.0, "size_pct": 40}, {"pct": 3.0, "size_pct": 30}, {"pct": 5.0, "size_pct": 30}]
        with patch("src.trade_executor.get_trading_client", return_value=bc), \
             patch("src.trade_executor.FeishuNotifier", return_value=notifier), \
             patch("src.trade_executor.count_active_positions", return_value=0), \
             patch("src.smart_order.SmartOrder", return_value=so), \
             patch("src.trade_executor.PortfolioManager"):
            from main import execute_auto_trade
            result = execute_auto_trade("SOLUSDT", 100.0, "trend", 2.0, tps, 98.0, 24, ["RSI"], "RSI", score=75)
        assert result["success"] is True

    def test_s19_sl_notional_qty_increased(self):
        result, _ = self._run(usdt_bal=200, filters={"step_size": 0.01, "qty_decimals": 2, "min_qty": 0.01, "min_notional": 20.0})
        assert result["success"] is True

    def test_s20_tp_skipped_notional_too_low(self):
        result, _ = self._run(usdt_bal=200, filters={"step_size": 0.01, "qty_decimals": 2, "min_qty": 0.01, "min_notional": 200.0})
        # Buy succeeds (min_notional check is in SmartOrder TP split, not execute_auto_trade)
        assert result["success"] is True

    def test_s21_stepsize_btc(self):
        result, _ = self._run(symbol="BTCUSDT", price=50000.0, filters={"step_size": 0.001, "qty_decimals": 3, "min_qty": 0.001})
        assert result["success"] is True
    ...
    def test_s25_qty_below_minqty(self):
        ...
        assert result["success"] is False
        assert "Caps reduced position below $10 minimum" in result["error"] or "Qty too small" in result["error"]
