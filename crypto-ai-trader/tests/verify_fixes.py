...
cps_match = re.search(r'def calculate_position_size\(.*?(?=\n    def |\nclass |\Z)', smart_order, re.DOTALL)
...
ca_match = re.search(r'def count_active_positions\(.*?(?=\ndef |\Z)', trade_executor, re.DOTALL)
...
ptu_match = re.search(r'def post_trade_update\(.*?(?=\n    def |\nclass |\Z)', risk_manager, re.DOTALL)
...
gsf_match = re.search(r'def get_symbol_filters\(.*?(?=\n    def |\Z)', smart_order, re.DOTALL)
gsf = gsf_match.group(0) if gsf_match else ""
check("no private _get_exchange_info in get_symbol_filters", "_get_exchange_info" not in gsf)
check("BinanceClient has public get_exchange_info", "def get_exchange_info(self)" in binance_client)
...
check("only one TP capping + reserve block", reserve_count == 1, f"found {reserve_count}")
...
check("no inline 'import time; time.sleep' remaining", "import time; time.sleep" not in trade_executor)
...
check("main.py: no inline 'import time; time.sleep' remaining", "import time; time.sleep" not in main_py)
...
ts_get_match = re.search(r'class TrailingStop:.*?def get\(self.*?(?=\n    def |\Z)', risk_manager, re.DOTALL)
...
gcp_match = re.search(r'def get_current_positions\(.*?(?=\n    def |\Z)', smart_order, re.DOTALL)
