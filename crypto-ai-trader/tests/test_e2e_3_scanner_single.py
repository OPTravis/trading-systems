...
def test_single_scan():
    ...
    print(f"[1] BinanceClient initialized")
    
    scanner = MarketScanner(binance_client=client)
    print(f"[2] MarketScanner initialized")
    ...
    if result is None:
        ...
        print("[PASS] Test 3 — Scanner ran without crashing (result=None is acceptable)")
    else:
        print(f"[3] Result: symbol={result['symbol']}, score={result['score']}, entry_signal={result.get('entry_signal')}")
