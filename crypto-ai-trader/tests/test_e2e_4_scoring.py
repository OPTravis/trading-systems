...
def test_scoring():
    ...
    sentiment_data = {"sentiment_score": 55, "funding_rate": 0.0001, "oi_change_pct": 2.5}
    coin_data = {"price": 104000, "volume_24h": 5e9, "price_change_24h": 3.2, "volume_surge": True}
    new_signals_data = {
    ...
    if factor_scores:
        print(f"[4] Factor scores: {json.dumps({k: round(v, 2) if isinstance(v, (int, float)) else v for k, v in factor_scores.items()}, indent=2)}")
