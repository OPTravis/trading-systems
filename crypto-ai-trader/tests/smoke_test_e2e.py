...
test("PaperTrader.is_paper_mode() returns True",
...
def test_strategy_adaptor():
    ...
    assert "strategies" in result, f"Missing 'strategies' in adapt result"
    assert "global" in result, f"Missing 'global' in adapt result"
...
def test_top_movers():
    ...
    assert isinstance(movers, list), f"get_top_movers should return list"
