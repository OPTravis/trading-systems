    ...
    try:
        resp = requests.post(
            ...
            json={
                "model": "deepseek-chat",
                "messages": [
                    {"role": "system", "content": "You are a crypto sentiment rater. Return ONLY a JSON array of objects."},
        ...
        for s in scores:
            ...
            assert "score" in s, f"Missing 'score' key"
            assert "confidence" in s, f"Missing 'confidence' key"
...
def test_xiaomi_api():
    ...
    try:
        ...
        if not content and message.get("reasoning_content"):
            print(f"   ⚠️  mimo-v2.5 returned reasoning only (content empty)")
        ...
        if not content:
            print(f"   ❌ Empty response (finish_reason: {resp_json['choices'][0].get('finish_reason')})")
            return None
...
def test_cross_verification(deepseek_scores, xiaomi_scores):
    ...
    primary = [{"score": s["score"], "confidence": s["confidence"], "sentiment": (s["score"] - 5) / 5.0} for s in deepseek_scores]
    secondary = [{"score": s["score"], "confidence": s["confidence"], "sentiment": (s["score"] - 5) / 5.0} for s in xiaomi_scores]
    ...
    print(f"   Cross-verification results:")
    for i, r in enumerate(result):
        diff = abs(r["primary_score"] - r["secondary_score"])
        print(f"      Article {i+1}: primary={r['primary_score']}, secondary={r['secondary_score']}, "
    ...
    print(f"   ✅ Cross-verification logic correct")
    return True
