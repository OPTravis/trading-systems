...
def mock_binance_client():
    ...
    client.get_symbol_filters = MagicMock(
        return_value={
            "minQty": 0.001,
            "maxQty": 100000,
            "stepSize": 0.001,
...
class TestSwitchDecision:
    ...
    def test_loss_threshold_triggers_switch(
        self, optimizer, mock_portfolio, mock_scanner, mock_binance_client
    ):
        """Position with -6% 24h change should trigger switch."""
        mock_binance_client.get_24hr_stats = MagicMock(
            return_value={"price_change_pct": -6.0}
        )
        mock_portfolio.get_all_positions = MagicMock(
            return_value=[
                {
                    "symbol": "BTCUSDT",
                    "quantity": 0.1,
        ...
        mock_scanner.scan_all = MagicMock(
            return_value=[
                {
                    "symbol": "BTCUSDT",
                    "price": 47000,
    ...
    def test_score_gap_triggers_switch(
        self, optimizer, mock_portfolio, mock_scanner, mock_binance_client
    ):
        """New coin score 25 points higher should trigger switch."""
        mock_binance_client.get_24hr_stats = MagicMock(
            return_value={"price_change_pct": -2.0}
        )
        mock_portfolio.get_all_positions = MagicMock(
            return_value=[
                {
    ...
    def test_blacklist_24h_change_30_skipped(
        self, optimizer, mock_portfolio, mock_scanner, mock_binance_client
    ):
    ...
    def test_cooldown_blocks_switch(
        self, optimizer, mock_portfolio, mock_scanner, mock_binance_client
    ):
        ...
        mock_binance_client.place_market_sell = MagicMock(
            return_value={"orderId": 11111}
        )
        mock_binance_client.place_market_buy = MagicMock(
            return_value={"orderId": 22222}
        )
        decisions = optimizer.analyze_and_switch(dry_run=True)
...
class TestExecuteSwitch:
    ...
    def test_successful_switch_updates_portfolio(
        self, optimizer, mock_binance_client, mock_portfolio
    ):
    ...
    def test_sell_fails_aborts_switch(
        self, optimizer, mock_binance_client, mock_portfolio
    ):
    ...
    def test_buy_fails_half_failed_logged(
        self, optimizer, mock_binance_client, mock_portfolio
    ):
    ...
    def test_stepsize_flooring(self, optimizer, mock_binance_client, mock_portfolio):
        ...
        mock_binance_client.get_symbol_filters = MagicMock(
            return_value={
                "minQty": 0.001,
                "stepSize": 0.001,
                "minNotional": 10,
        ...
        qty = call_args.kwargs.get(
            "quantity", call_args.args[1] if len(call_args.args) > 1 else 0
        )
        # Should be floored to 3 decimals (0.001 step)
        assert round(qty, 3) == qty, f"Quantity {qty} not floored to stepSize"

    def test_portfolio_update_failure_non_critical(
        self, optimizer, mock_binance_client, mock_portfolio
    ):
...
class TestDryRun:
    ...
    def test_dry_run_no_orders_placed(
        self, optimizer, mock_binance_client, mock_portfolio, mock_scanner
    ):
    ...
    def test_dry_run_false_executes(
        self, optimizer, mock_binance_client, mock_portfolio, mock_scanner
    ):
...
class TestCooldownPersistence:
    ...
    def test_switch_times_saved_to_statedb(
        self, optimizer, mock_portfolio, mock_binance_client
    ):
