...
def test_klines():
    ...
    print(f"[1] BinanceClient initialized OK")
    ...
    if klines:
        ...
        print(f"    symbol={sample.get('symbol')}, open={sample.get('open')}, close={sample.get('close')}, volume={sample.get('volume')}")
        assert "open" in sample and "close" in sample and "volume" in sample, "Missing required fields"
        print(f"    (note: 'symbol' key absent in kline dict — normal for ccxt client)")
