...
def test_specialist_result_custom():
    r = SpecialistResult(
        score=75.5, signals=["test"], data={"k": "v"}, confidence="high"
    )
    assert r.score == 75.5
...
def test_technical_agent_with_precomputed():
    ...
    print(
        f"  [technical/precomputed] score={result.score}, signals={len(result.signals)}"
    )
...
def test_technical_agent_with_klines():
    ...
    try:
        ...
        assert isinstance(result.data, dict)
        print(
            f"  [technical/klines] score={result.score}, signals={len(result.signals)}"
        )
...
def test_technical_agent_factor_technical_subscores():
    ...
    assert (
        TechnicalAgent._factor_technical({"macd_histogram": 1.0}) == 35
    )  # 10 (RSI default 50) + 25 (MACD)
...
def test_technical_agent_factor_price_action_subscores():
    # Ideal volatility
    assert (
        TechnicalAgent._factor_price_action({"volatility_pct": 5.0, "momentum": 3.0})
        == 100
    )
    # No momentum
    assert (
        TechnicalAgent._factor_price_action({"volatility_pct": 5.0, "momentum": 0.0})
        == 80
    )
...
def test_technical_agent_factor_bb_squeeze():
    ...
    assert (
        TechnicalAgent._factor_bb_squeeze({"squeezing": True, "percentile": 10}) == 80.0
    )
    assert (
        TechnicalAgent._factor_bb_squeeze({"squeezing": False, "percentile": 50})
        == 30.0
    )
...
def test_technical_agent_factor_rsi_divergence():
    ...
    assert (
        TechnicalAgent._factor_rsi_divergence({"detected": True, "strength": 50})
        == 85.0
    )
    assert (
        TechnicalAgent._factor_rsi_divergence({"detected": False, "rsi_current": 25})
        == 50.0
    )
...
def test_volume_agent_subscore_price_change():
    ...
    assert (
        VolumeAgent._factor_volume_momentum({"price_change_24h": 3.0}) == 25
    )  # 5 + 20
    assert (
        VolumeAgent._factor_volume_momentum({"price_change_24h": 10.0}) == 35
    )  # 5 + 30
    assert (
        VolumeAgent._factor_volume_momentum({"price_change_24h": 20.0}) == 20
    )  # 5 + 15
    assert (
        VolumeAgent._factor_volume_momentum({"price_change_24h": -1.0}) == 15
    )  # 5 + 10


def test_volume_agent_surge_bonus():
    base = VolumeAgent._factor_volume_momentum({"rank": 50, "price_change_24h": 0})
    with_surge = VolumeAgent._factor_volume_momentum(
        {"rank": 50, "price_change_24h": 0, "volume_surge": True}
    )
...
def test_sentiment_agent_positive():
    ...
    result = agent.analyze(
        funding_data={"sentiment_score": 10, "funding_rate": -0.02, "oi_change_pct": 15}
    )
    assert 0 <= result.score <= 100
...
def test_sentiment_agent_neutral():
    agent = SentimentAgent()
    result = agent.analyze(
        funding_data={"sentiment_score": 0, "funding_rate": 0.01, "oi_change_pct": 0}
    )
    assert result.score == 50.0
...
def test_fng_contrarian_mapping_all_ranges():
    ...
    for fng, expected in cases:
        ...
        ), f"F&G={fng}: expected {expected}, got {actual}"
...
def test_prepump_obv_divergence_detected():
    agent = PrePumpAgent()
    result = agent.analyze(
        obv_div_data={"detected": True, "strength": 60, "obv_trend": "rising"}
    )
    assert 0 <= result.score <= 100
...
def test_prepump_subscore_obv_divergence():
    ...
    assert (
        PrePumpAgent._factor_obv_divergence(
            {"detected": True, "strength": 100, "obv_trend": "rising"}
        )
        == 100.0
    )
    assert (
        PrePumpAgent._factor_obv_divergence({"detected": False, "obv_trend": "rising"})
        == 50.0
    )
    assert (
        PrePumpAgent._factor_obv_divergence({"detected": False, "obv_trend": "falling"})
        == 30.0
    )
...
def test_prepump_subscore_consolidation():
    ...
    assert (
        PrePumpAgent._factor_consolidation(
            {"breaking_out": True, "volume_confirmed": True}
        )
        == 95.0
    )
    ...
    assert (
        PrePumpAgent._factor_consolidation(
            {"breaking_out": True, "volume_confirmed": False}
        )
        == 80.0
    )
    ...
    assert (
        PrePumpAgent._factor_consolidation(
            {"in_consolidation": True, "days_in_range": 45, "range_pct": 10}
        )
        == 75.0
    )
    ...
    assert (
        PrePumpAgent._factor_consolidation(
            {"in_consolidation": True, "days_in_range": 20, "range_pct": 20}
        )
        == 50.0
    )
...
def test_combined_weighted_score():
    ...
    r_vol = VolumeAgent().analyze(
        coin_data={"rank": 10, "price_change_24h": 5, "volume_surge": True}
    )
    r_sent = SentimentAgent().analyze(
        {"sentiment_score": 5, "funding_rate": -0.01, "oi_change_pct": 8}
    )
