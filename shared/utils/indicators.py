...
class Indicators:
    ...
    def vwap(klines: List[Dict]) -> float:
        ...
        total_pv = sum(((k["high"] + k["low"] + k["close"]) / 3) * k["volume"] for k in klines)
    ...
    def adx(klines: List[Dict], period: int = 14) -> float:
        ...
        smoothed_tr = sum([Indicators._true_range(klines[i], klines[i-1]) for i in range(1, period + 1)]) / period
        ...
        for i in range(period, len(plus_dm)):
            ...
            smoothed_tr = (smoothed_tr * (period - 1) + Indicators._true_range(klines[i + 1], klines[i])) / period
    ...
    def obv_divergence(klines: List[Dict], lookback: int = 20) -> Dict:
        ...
        strength = 0.0
        if detected:
            price_drop = (min(first_half_prices) - min(second_half_prices)) / min(first_half_prices) * 100
            obv_rise = (min(second_half_obv) - min(first_half_obv)) / abs(min(first_half_obv)) * 100 if min(first_half_obv) != 0 else 0
    ...
    @staticmethod
    def rsi_divergence(klines: List[Dict], period: int = 14, lookback: int = 30) -> Dict:
        ...
        if len(rsi_vals) < lookback:
            return {"detected": False, "strength": 0, "rsi_current": rsi_vals[-1] if rsi_vals else 50}
        ...
        return {"detected": detected, "strength": strength, "rsi_current": rsi_vals[-1] if rsi_vals else 50}

    @staticmethod
    def consolidation_breakout(klines: List[Dict], min_days: int = 30, max_range_pct: float = 25.0) -> Dict:
        ...
        if len(klines) < min_days + 5:
            return {"in_consolidation": False, "breaking_out": False,
                    "range_pct": 0, "days_in_range": 0, "volume_confirmed": False}
        ...
        volume_confirmed = recent_vol > range_vol_avg * 1.5 if range_vol_avg > 0 else False
    ...
    def btc_trend_score(klines: List[Dict]) -> Dict:
        ...
        if len(klines) < 60:
            return {
                "score": 50, "trend": "NEUTRAL", "allow_long": True,
                "factors": {},
                "ema_21": 0, "ema_55": 0, "rsi_14": 50, "macd_hist": 0,
                "sma_200": 0, "sma_50": 0, "btc_close": 0,
            }
